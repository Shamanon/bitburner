...
export async function main(ns) {
  ...
  while (true) {
    ...
    if (button !== null) {
      let faction = doc.querySelector("#root > div.jss1.MuiBox-root.css-0 > div > div > div.MuiPaper-root.MuiPaper-elevation.MuiPaper-rounded.MuiPaper-elevation1.css-1p7xay4 > div > div > div > input");
      ns.print(faction.value + " found button!");
      while (true) {
        ns.print(faction.value);
        await ns.sleep(1000);
        if(faction.value == "Slum Snakes") continue;
      }
      faction.value = "Slum Snakes";
      //ns.exit();
      await button.click();
    }
