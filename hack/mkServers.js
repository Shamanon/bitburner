...
export async function main(ns) {
  ...
  while (keepGoing) {
    ...
    for (let i = 0; i < servers.length; i++) {
      ...
      if (highestRAM < currentRAM) highestRAM = currentRAM;

      servernumber = (servers[i].slice(servers[i].length - 2));
      let servernumber3digit = (servers[i].slice(servers[i].length - 3));

      if (parseInt(servernumber3digit) > parseInt(servernumber)) servernumber = servernumber3digit;

      if (parseInt(servernumber) > highestserver) highestserver = parseInt(servernumber);
