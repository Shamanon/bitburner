  ...
  while (true) {
    ...
    for (var server of servers) {
      ...
      if (!pwnd.includes(server) && !isPwnd &&
        ...
        ns.getServerNumPortsRequired <= portTakers) {
        pwn(ns,server);
        pwnd.push(server);
        ns.print(server + ' PWN!');
      ...
      if (maxMoney[server] > earnableThreshold && !earnable.includes(server)) earnable.push(server);
    }
    //pwnd.push('home');
      ...
      for (let i = 0; i < pwnd.length; i++) {
        ...
        if (maxThreads >= 1) {
          for (let i2 = 0; i2 < earnable.length; i2++) {
            ...
            if(maxThreads < ns.getScriptRam(script)) break;
            if(maxThreads < usedServerRAM) break;
            myCount++;
