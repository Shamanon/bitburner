...
export async function main(ns) {
    ...
    var earnableThreshold = 10000;
    var keepFree = ns.getServerMaxRam('home') * .1;
    if (arg.debug)
        ns.tail();
    if (arg.debug)
    ...
    while (true) {
        ...
        if (earnable.length > 0) { //Don't do this if there are no targets.
            ...
            for (let i = 0; i < pwnd.length; i++) {
                if(pwnd[i].includes("hacknet-server") && !arg.hnet) continue;
                ...
                if (pwnd[i] == 'home' && usableRAM < keepFree)
                    continue;
                if (pwnd[i] == 'home')
                ...
                if (arg.debug) {
                    ns.print(hackMe);
                    ns.print(totalCash + ':' + totalThreads);
                }
                for (let i2 = 0; i2 < hackMe.length; i2++) {
                    ...
                    script_args[1] = myThreads;
                    //hack server
                    ns.exec(script, pwnd[i], myThreads, ...script_args);
                    if (arg.debug)
                        ns.print('Hacking ' + server + ' from ' + pwnd[i] + ' with ' + myThreads + ' threads.');
                    //else ns.toast('Hacking ' + server + ' from ' + pwnd[i] + ' with ' + myThreads + ' threads.');
                }
                //show info on hack
                if (myCount > 0) {
                    if (arg.debug)
                        ns.print('Hacking ' + myCount + ' servers from ' + pwnd[i]);
                    else
                        ns.toast('Hacking ' + myCount + ' servers from ' + pwnd[i]);
                }
            }
        }
